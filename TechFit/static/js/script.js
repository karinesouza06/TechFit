...
document.addEventListener('DOMContentLoaded', () => {
    ...
    tipoUsuario.addEventListener('change', () => {
        ...
        if(value === 'aluno') {
            ...
            alunoFields.querySelectorAll('select, input').forEach(el => el.required = true);
        } 
        else if(value === 'personal') {
            personalFields.classList.remove('hidden');
            personalFields.querySelectorAll('select, input').forEach(el => el.required = true);
        }
    });
...
document.getElementById('tipoUsuario').addEventListener('change', function() {
    ...
    document.querySelectorAll('.aluno-fields, .personal-fields').forEach(div => {
        div.classList.add('hidden');
        div.querySelectorAll('[required]').forEach(input => input.removeAttribute('required'));
    });
    
    if(tipo === 'aluno') {
        document.querySelector('.aluno-fields').classList.remove('hidden');
        document.querySelectorAll('.aluno-fields [required]').forEach(input => input.setAttribute('required', true));
    }
    else if(tipo === 'personal') {
        document.querySelector('.personal-fields').classList.remove('hidden');
        document.querySelectorAll('.personal-fields [required]').forEach(input => input.setAttribute('required', true));
    }
});